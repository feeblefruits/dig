jQuery(document).ready(function($){
	...
	var ts_lesotho = new TwoStep({
	    elements_lesotho: document.querySelectorAll('.cd-section-lesotho'),
	    onChange: function(event) {

	    	for (let i = 0; i < contentListLesotho.length; i++) {
	        	console.log(contentListLesotho[i]);
	    	}
	    },
	    stick: document.querySelector('.sticky-outer-lesotho'),
	    narrative: [
	        function(event) {
	            $('.floating-box-lesotho').html(contentListLesotho[0]);
	        },
	        function(event) {
	            $('.floating-box-lesotho').html(contentListLesotho[1]);
	        },
	        function(event) {
	            $('.floating-box-lesotho').html(contentListLesotho[2]);
	        },
	        function(event) {
	            $('.floating-box-lesotho').html(contentListLesotho[3]);
	        },
	        function(event) {
	            $('.floating-box-lesotho').html(contentListLesotho[4]);
	        },
	        function(event) {
	            $('.floating-box-lesotho').html(contentListLesotho[5]);
	        },
	        function(event) {
	            $('.floating-box-lesotho').html(contentListLesotho[6]);
	        },
	        function(event) {
	            $('.floating-box-lesotho').text(contentListLesotho[7]);
	        },
	        function(event) {
	            $('.floating-box-lesotho').text(contentListLesotho[8]);
	        },
	        function(event) {
	            $('.floating-box-lesotho').text(contentListLesotho[9]);
	        },
	        function(event) {
	            $('.floating-box-lesotho').text(contentListLesotho[10]);
	        },
	        function(event) {
	            $('.floating-box-lesotho').text(contentListLesotho[11]);
	        },
	        function(event) {
	            $('.floating-box-lesotho').text(contentListLesotho[12]);
	        },
	        function(event) {
	            $('.floating-box-lesotho').text(contentListLesotho[13]);
	        }
	    ],
	    offset: {
	        up: '-80%',
	...
	//variables
	var hijacking= $('.stories-lesotho').data('hijacking'),
		animationType = $('.stories-lesotho').data('animation'),
		delta = 0,
        scrollThreshold = 5,
        actual = 1,
        animating = false;
    ...
    //DOM elements
    var sectionsAvailable = $('.cd-section-lesotho'),
    	verticalNav = $('.cd-vertical-nav-lesotho'),
	...
	function initHijacking() {
		...
		var visibleSection = sectionsAvailable.filter('.visible'),
			topSection = visibleSection.prevAll('.cd-section-lesotho'),
			bottomSection = visibleSection.nextAll('.cd-section-lesotho'),
    ...
    function prevSection(event) {
    	...
    	visibleSection = middleScroll ? visibleSection.next('.cd-section-lesotho') : visibleSection;

    	var animationParams = selectAnimation(animationType, middleScroll, 'prev');
    	unbindScroll(visibleSection.prev('.cd-section-lesotho'), animationParams[3]);

        if( !animating && !visibleSection.is(":first-child") ) {
        	animating = true;
            visibleSection.removeClass('visible').children('div').velocity(animationParams[2], animationParams[3], animationParams[4])
            .end().prev('.cd-section-lesotho').addClass('visible').children('div').velocity(animationParams[0] , animationParams[3], animationParams[4], function(){
            	animating = false;
    ...
    function nextSection(event) {
        ...
        if(!animating && !visibleSection.is(":last-of-type") ) {
            ...
            .end().next('.cd-section-lesotho').addClass('visible').children('div').velocity(animationParams[0], animationParams[3], animationParams[4], function(){
            	animating = false;
            	if( hijacking == 'off') $(window).on('scroll', scrollAnimation);
            });
    ...
    function unbindScroll(section, time) {
    	...
    	if( hijacking == 'off') {
    		$(window).off('scroll', scrollAnimation);
    		( animationType == 'catch') ? $('.stories-lesotho').scrollTop(section.offset().top) : section.velocity("scroll", { duration: time });
    	}
    }
	...
	function deviceType() {
		...
		return window.getComputedStyle(document.querySelector('.stories-lesotho'), '::before').getPropertyValue('content').replace(/"/g, "").replace(/'/g, "");
	}
